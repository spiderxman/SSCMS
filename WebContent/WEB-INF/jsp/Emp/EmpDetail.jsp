...
<fieldset><legend>基本情報 </legend>
    <div>社員ID
         <label>${empInfo.empId}</label>
         <input type="hidden" id="empBaseBean.empId" name="empBaseBean.empId" value="${empInfo.empId}">
    </div>
    <div>社員名称（姓）
            <input style="width: 120px" id="empBaseBean.empFirstName"
                class=inputText name="empBaseBean.empFirstName" maxLength=20 placeholder="例）abc" value="${empInfo.empFirstName}"/>*
                    社員名称（名）
            <input style="width: 120px" id="empBaseBean.empLastName"
                class=inputText name="empBaseBean.empLastName" maxLength=20 placeholder="例）abc" value="${empInfo.empLastName}"/>*
    </div>
    <div>
         社員名称カタカナ
            <input style="width: 120px" id="empBaseBean.empNameKana"
                class=inputText name="empBaseBean.empNameKana" maxLength=20 value="${empInfo.empNameKana}"/>*
     社員名称（略）
            <input style="width: 120px" id="empBaseBean.empNickName"
                class=inputText name="empBaseBean.empNickName" maxLength=20 value="${empInfo.empNickName}"/>*
    </div>
         <tr>
        ...
        <div>
            ...
            <select id="empBaseBean.authority" class=ddlBlack name="empBaseBean.authority">
				<c:forEach var="item" items="${authorityCodeList}">
					<c:if test="${empInfo.authority==item.code}">
						<option selected value="${item.code}">${item.codeValue}</option>
					</c:if>
					<c:if test="${empInfo.authority!=item.code}">
        ...
        <div>
        	生年月日
        	<input type="text" id="birthday" name="empBaseBean.birthday" value="${empInfo.birthday}"/>*
        </div>
        <div>
        	国籍／出身
        	<input type="text" name="empBaseBean.country" value="${empInfo.country}"/>*
        </div>
        <div>
        	現住所
        	<input type="text" name="empBaseBean.address" value="${empInfo.address}"/>*
        </div>
        <div>
        	最寄駅
        	<input type="text" name="empBaseBean.station" value="${empInfo.station}"/>
        </div>
        <div>
        	電話番号
        	<input type="text" name="empBaseBean.tel" maxLength=15 value="${empInfo.tel}"/>
        </div>
        <div>
        	メールアドレス
        	<input type="text" name="empBaseBean.mail" maxLength=50 value="${empInfo.mail}"/>
        </div>
        <div>
        	最終学校
        	<input type="text" name="empBaseBean.school" maxLength=50 value="${empInfo.school}"/>
        </div>
        <div>
        	最終学歴
        ...
        <div>
        	専門
        	<input type="text" name="empBaseBean.major" maxLength=30 value="${empInfo.major}"/>
        </div>
        <div>
        	卒業年月
        	<input type="text" name="empBaseBean.graduateDate" maxLength=8 value="${empInfo.graduateDate}"/>
        </div>
        <div>
        	在留資格
        ...
        <div>
        	外語１
        	<input type="text" name="empBaseBean.language1" maxLength=20 value="${empInfo.language1}"/>
        	レベル１
        	<input type="text" name="empBaseBean.level1" maxLength=20 value="${empInfo.level1}"/>
        </div>
        <div>
        	外語２
        	<input type="text" name="empBaseBean.language2" maxLength=20 value="${empInfo.language2}"/>
        	レベル２
        	<input type="text" name="empBaseBean.level2" maxLength=20 value="${empInfo.level2}"/>
        </div>
        <div>
        	技術資格証明書
        	<input type="text" name="empBaseBean.certificate" maxLength=20 value="${empInfo.certificate}"/>
        </div>
        <div>
        	社員種別
